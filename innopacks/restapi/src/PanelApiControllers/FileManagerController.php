...
class FileManagerController extends BaseController
{
    ...
    private function getFileManagerData(): array
    {
        ...
        return [
            ...
            'title'       => plugin_setting('file_manager', 'driver') === 'oss' ? 'OSS File management' : 'Picture Space',
            'config'      => [
                'driver'   => plugin_setting('file_manager', 'driver', 'local'),
    ...
    public function getStorageConfig(): mixed
    {
        try {
            ...
            return json_success('Storage configuration successfully obtained', $config);
        } catch (\Exception $e) {
            Log::error('Failed to obtain storage configuration:', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);

            return json_fail('Failed to obtain storage configuration: '.$e->getMessage());
        }
    }
    ...
    public function saveStorageConfig(Request $request): mixed
    {
        try {
            ...
            return json_success('Storage configuration saved successfully', $configData);
        } catch (\Exception $e) {
            Log::error('Failed to save storage configuration:', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);

            return json_fail('Failed to save storage configuration: '.$e->getMessage());
        }
    }
}
