...
class LeadContactRepo extends BaseRepo
{
     ...
     * @throws Throwable
     */
    private function createOrUpdate(LeadContact $contactlead, $data): void
    {
        DB::beginTransaction();

        try {
            $leadData = $this->handleData($data);
            $contactlead->fill($leadData);
            $contactlead->saveOrFail();
            DB::commit();
        } catch (Exception $e) {
            DB::rollBack();
            throw $e;
