...
if (! function_exists('innoshop_brand_link')) {
    ...
    {
        if (is_admin()) {
            $default = '<a href="/" class="ms-2" target="_blank">Urban Grain</a>';
        } else {
            $default = '<a href="/" class="ms-2" target="_blank">Urban Grain</a>';
        }
...
if (! function_exists('theme_mix')) {
    ...
    if (! function_exists('getFivewords')) {
        function getFivewords($inputstring, $count = 5) {
    ...
    if (! function_exists("getDiscountPercentage")) {
        function getDiscountPercentage($originalPrice, $discountedPrice) {
            ...
            return round($discount); // Round to 2 decimal places
        }    
    }
    ...
    if (! function_exists("getAvgRating")) {
        ...
        {
            // $productId = 11;
            return round(Review::where('product_id', $productId)->avg('rating'),1) ?? 0;
