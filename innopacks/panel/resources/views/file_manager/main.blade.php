...
@section('content')
      ...
      <el-col :span="12">
        <el-button-group>
          ...
          <el-button type="primary" size="small" @click="handleConfirm">
            {{ __('panel/file_manager.select_submit') }}
          </el-button>
        </el-button-group>
      </el-col>
  ...
  <el-row :gutter="0">
          ...
          >
            <span class="custom-tree-node">
              <i
                ...
                style="margin-right: 4px; color: #8446df;"
              ></i>
    ...
    */ ?>
          ...
          <div v-loading="loading" element-loading-text="Loading...">
            <el-row :gutter="20">
              ...
              >
                <div
                  :class="[
                    'file-card',
                    selectedFiles.includes(file.id || file.path) ? 'selected' : ''
                  ]"
                    ...
                    <template v-else>
                      <div class="file-thumb">
                        ...
                        <el-image
                          ...
                          :alt="file.name"
                          fit="contain"
                          :preview-src-list="[file.origin_url || file.url]"
                        ></el-image>
                      </div>
                    </template>
                  </div>
            ...
            <div class="pagination-container">
              <el-pagination
                ...
                prev-text="{{ __('panel/file_manager.prev') }}"
                next-text="{{ __('panel/file_manager.next') }}"
            ...
            <el-empty
              v-else
              :description="'{{ __('panel/file_manager.empty') }}'"
              :image-size="120"
            ></el-empty>
          </div>
        </div>
    ...
    <el-dialog title="Create New Folder" :visible.sync="folderDialog.visible" width="400px">
    ...
    <el-dialog title="Upload File" :visible.sync="uploadDialog.visible" width="500px" @open="onUploadDialogOpen">
      ...
      >
        ...
        <div class="el-upload__tip" slot="tip">
          Supports jpg, jpeg, png, gif, webp, mp4 file formats
        </div>
        <div class="el-upload__tip" slot="tip">
          Maximum upload file size on server: {{ $uploadMaxFileSize ?? 'Unknown' }}
        </div>
        <div class="el-upload__tip" slot="tip">
          Maximum POST data size: {{ $postMaxSize ?? 'Unknown' }}
        </div>
      </el-upload>
    </el-dialog>
    ...
    <div
      ...
      :style="{ top: folderContextMenu.style.top, left: folderContextMenu.style.left }"
    >
      <ul>
      ...
      methods: {
        ...
        beforeUpload(file) {
          ...
          if (!this.uploadData.path) {
            this.$message.error('Upload path is not defined, please select a folder');
            return false;
          }
        ...
        handleTreeDrop(event, node, data) {
          ...
          if (this.isDragging && this.draggedFile) {
            ...
            http.post('file_manager/move_files', {
              ...
              this.$message.error(err.message || 'Failed to move');
            }).finally(() => {
              this.isDragging = false;
              this.draggedFile = null;
            });
          ...
          axios.post('/api/panel/file_manager/storage_config', this.storageConfig)
            .then(response => {
              if (response.data && response.success) {
                ...
                layer.msg(response.data ? response.message : 'Failed to save', {
                  icon: 2
                });
              }
            })
        ...
        getStorageConfig() {
          axios.get('/api/panel/file_manager/storage_config')
            ...
            .catch(error => {
              console.error('Request to retrieve storage configuration failed:', error);
              // 设置默认值
              this.storageConfig = {
                driver: 'local',
                key: '',
