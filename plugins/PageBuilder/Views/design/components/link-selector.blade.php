  ...
  <div class="link-selector-wrap">
    <div class="selector-type" @blur="selectorContentShow = false" tabindex="1">
      <div class="title" v-if="!link.type || link.type === '' || !value.value"
        @click="toggleSelector">Please select link type
      </div>
    ...
    <el-dialog :visible.sync="linkDialog.show" class="link-dialog-box" :append-to-body="true"
      ...
      <div slot="title" class="link-dialog-header">
        <div class="title">Select @{{ dialogTitle }}</div>
        <div class="input-with-select" v-if="link.type != 'custom'">
          <input type="text" placeholder="Please enter keyword to search" v-model="keyword" @keyup.enter="searchProduct"
            class="form-control">
          <el-button @click="searchProduct"><i class="el-icon-search"></i> Search</el-button>
        </div>
      </div>
      <div class="link-dialog-content">
        <div class="product-search">
          ...
          <a :href="linkTypeAdmin" target="_blank"
            v-if="link.type != 'custom' && link.type != 'static'">Manage @{{ dialogTitle }}</a>
        </div>

        <div class="link-text" v-if="isCustomName">
          <div class="module-edit-group edit-group-margin">
            <div class="module-edit-title">Custom Name</div>
            <text-i18n v-model="link.text"></text-i18n>
          </div>
        </div>
        <template v-if="link.type == 'custom'">
          <div class="linkDialog-custom">
            <el-input v-model="link.value" placeholder="Please enter link address"></el-input>
          </div>
        </template>
        ...
        <template v-else>
          <div class="product-info" v-loading="loading">
              ...
              <ul class="product-list">
                <li v-for="(product, index) in linkDialog.data"
                  ...
                  <div :class="'status-cell ' + (product.active ? 'ok' : 'no')">
                    <template v-if="product.active">Enable</template>
            ...
            <div class="product-info-no" v-if="!linkDialog.data.length && loading === false">
              ...
              <div class="no-text">Data does not exist or has been deleted, <a :href="linkTypeAdmin" target="_blank">Go to add @{{ dialogTitle }}</a>
              </div>
            </div>
          </div>
        </template>
      ...
      <div slot="footer" class="link-dialog-footer">
        <el-button type="primary" @click="linkDialogConfirm">Sure</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  Vue.component('link-selector', {
    ...
    computed: {
      dialogTitle: function() {
        ...
        return foundType ? foundType.label : 'Select link';
      },
      selectorTitle() {
        ...
        return foundType ? foundType.label : 'Unknown type';
      },
      ...
      updateData() {
        ...
        axios.get(url, null, {
          ...
          if (res && res.data) {
            ...
            self.name = [{
              name: 'Data does not exist or has been deleted.'
            }];
          }
        }).catch((error) => {
