    ...
    methods: {
      ...
      moduleUpdated: inno.debounce(function(val) {
        ...
        axios.post(url + '?design=1', data).then((res) => {
          ...
          if (error.response) {
            ...
            if (status === 404) {
              errorMessage = 'Module template file not found, please contact administrator';
            } else if (status === 500) {
              errorMessage = 'Internal server error, please try again later';
            } else if (status === 422) {
              errorMessage = 'Module data format error: ' + (data.message || 'Unknown error');
      ...
      editModuleButtonClicked(index) {
        if (this.design) {
          ...
          if (this.design.editingModuleIndex === index && this.design.editType === 'module') {
            console.log('Already editing the current module, skipping duplicate processing', index);
            return;
          }
      ...
      saveButtonClicked() {
        ...
        axios.put(url, this.form).then((res) => {
          ...
          layer.msg('Save Failed: ' + (error.response?.data?.message || error.message), {icon: 2});
        });
      },
      ...
      importDemoData() {
        const page = '{{ $page ?? "home" }}';
        if (page !== 'home') {
          layer.msg('Demo data is only supported for the home page');
          return;
        }
        
        if (confirm('Are you sure you want to import demo data? This will overwrite the current page design.')) {
          ...
          axios.post(url).then((res) => {
            ...
            layer.msg('Import Failed: ' + (error.response?.data?.message || error.message));
          });
        }
      },
