...
<?php // Biling methods are hidden for now as there is only one method (Razorpay) and it requires additional fields to be shown in the checkout page, which are not implemented yet. Once we have more billing methods or the Razorpay integration is complete, we can uncomment this section and make necessary adjustments.
 /*
          <div class="checkout-item">
            <div class="title-wrap">
      ...
      <div class="col-12 col-md-5">
        <div class="checkout-data">
          ...
          */  ?>
            ...
            <button class="btn btn-primary btn-lg fw-bold w-100 to-checkout" type="button"
              @click="submitCheckout">{{ __('front/checkout.place_order') }}
            </button>
          </div>
        </div>
    ...
    const checkoutApp = createApp({
      setup() {
        ...
        const submitCheckout = () => {
          ...
          console.log("Submitting checkout with data:", current);
          // console.log("API endpoint:", api.checkoutConfirm);
          current.billing_method_code= "razorpay"; // Set Razorpay as the billing method for testing
          
          axios.post(api.checkoutConfirm, current).then(function(res) {
            if (res.success) {
              // console.log("res order number:", res.data.site_order_number);
              // console.log("res:", res);
              if(res.data.rzrpayment_order_id) {
                // Start payment process with Razorpay
                var options = {
                    key: "{{ config('razorpay.key') }}",
                    amount: `${res.data.amount * 100}`, // Convert to paise
                    currency: "INR",
                    order_id:  `${res.data.rzrpayment_order_id}`,
                    handler: function (response){
                      const resData = { order_id: res.data.order_id, response: response };
                      fetch("{{ route('razorpay.update_order_payment') }}", {
                            method: "POST",                            
                            headers: {
                        ...
                        .then(res => res.text())
                        .then((data) => {
                          location.href = inno.getBase() + '/razorpay/' + res.data.site_order_number + '/success';    
                ...
                rzp.on('payment.failed', function (response){
                  location.href = inno.getBase() + '/razorpay/' + res.data.site_order_number + '/failed';
              ...
              //   location.href = inno.getBase() + '/razorpay/' + res.data.number + '/pay'
              // })
            }
          }).finally(function() {
