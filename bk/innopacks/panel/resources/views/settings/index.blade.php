      ...
      <div class="card" id="setting-menu">
        ...
        <div class="card-body">
          <ul class="nav flex-column settings-nav">
            <a class="nav-link active" href="#" data-bs-toggle="tab" data-bs-target="#tab-setting-basics">
              {{ __('panel/setting.basic') }}
            </a>
            <a class="nav-link" href="#" data-bs-toggle="tab" data-bs-target="#tab-setting-webdata">
              {{ __('panel/setting.website_data') }}
            </a>
            <a class="nav-link" href="#" data-bs-toggle="tab" data-bs-target="#tab-setting-image">
              {{ __('panel/setting.image_settings') }}
            </a>
            <a class="nav-link" href="#" data-bs-toggle="tab" data-bs-target="#tab-setting-email">
              {{ __('panel/setting.email_setting') }}
            </a>
            <a class="nav-link" href="#" data-bs-toggle="tab" data-bs-target="#tab-setting-ai">
              {{ __('panel/setting.ai_setting') }}
            </a>
            <a class="nav-link" href="#" data-bs-toggle="tab" data-bs-target="#tab-setting-logistics-information">
              {{ __('panel/setting.express_company') }}
            </a>
...
<script>
  ...
  function getCountries() {
    axios.get('{{ front_route('countries.index') }}').then(function(res) {
      ...
      countrySelect.empty();
      countrySelect.append('<option value="">Please select a country</option>');
  ...
  // Get provinces/states by country (e.g. countries/72)
  function getZones(countryId) {
    axios.get('{{ front_route('countries.index') }}/' + countryId).then(function(res) {
      ...
      zoneSelect.append('<option value="">Please select a state/province</option>');
      zones.forEach(function(zone) {
        zoneSelect.append(
          '<option value="' + zone.code + '"' +
          (zone.code == stateCode ? ' selected' : '') +
          '>' + zone.name + '</option>'
        );
      });
    });
            ...
            ${locales.map((locale, locale_index) => `
              <div class="accordion-item">
                <h2 class="accordion-header">
                  <button class="accordion-button py-2 ${locale_index === 0 ? '' : 'collapsed'}"
                          type="button"
                          data-bs-toggle="collapse"
                          data-bs-target="#data-locale-${index}-${locale.code}">
                    <div class="wh-20 me-2">
                      <img src="${locale.image}" class="img-fluid">
                    </div>
                    ${locale.name}
                  </button>
                </h2>
                <div id="data-locale-${index}-${locale.code}"
                     class="accordion-collapse collapse ${locale_index === 0 ? 'show' : ''}"
                     data-bs-parent="#accordion-slideshow-${index}">
  ...
  $(document).on('click', '.is-up-file.slideshow-img .img-upload-item', function () {
    ...
    $('#form-upload input[name="file"]').change(function () {
      ...
      inno.imgUploadAjax(file, _self, (data) => {
        ...
        _self.find('.img-info').html(
          '<img src="' + data.data.url + '" class="img-fluid">'
        );
      });
    });
  });
    ...
    if ($activeTab.length) {
      ...
      if (target) {
        ...
        if ($('#current_tab').length === 0) {
          $(this).append(
            '<input type="hidden" name="tab" id="current_tab" value="' + tabId + '">'
          );
        } else {
