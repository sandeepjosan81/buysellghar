            ...
            <div class="modal-body">
              <div class="card bg-light">
                <div class="card-body">
                  <?php 
                  // echo "<pre>";
                  // print_r($payment->reference);
                  // echo "</pre>"; 
                  ?>
                  <table class="table table-bordered">
                    <tbody>
                    @foreach ($payment->reference as $key => $value)
                      <tr>
                        <th>{{ $key }}</th>
                        <td>
                          @if(is_array($value))
                            <pre class="mb-0 small"><code>{{ json_encode($value, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</code></pre>
                          @else
                            <pre class="mb-0 small"><code>{{ $value }}</code></pre>
                          @endif
                        </td>
                      </tr>
                    @endforeach
