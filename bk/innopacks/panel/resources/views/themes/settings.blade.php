...
@push('footer')
  <script>
    ...
    // Homepage recommended products management (organized by category）
            ...
            <div class="card-body">
              <div class="mb-2">
                <button type="button" class="btn btn-sm btn-primary add-product-to-category" data-group-index="${groupIndex}">
                  <i class="bi bi-plus-circle"></i> Add Product to This Category
                </button>
              </div>
    ...
    // Format categories as select dropdown options
    ...
    function renderSelectedHomeCategories() {
      ...
      if (selectedIds.length === 0) {
        selectedDiv.html('<p class="text-muted small">' + '{{ __('panel/setting.no_categories_selected') ?? "Select category" }}' + '</p>');
        return;
      }
